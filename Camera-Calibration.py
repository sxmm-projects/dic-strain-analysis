        ...
        for row in img.findall(".//ParameterList[@name]"):
            ...
            if x_values is not None and y_values is not None:
                ...
                if len(x_values) == len(y_values):
                    ...
                    print(f"[Warning] Skipping {row.attrib['name']} in {img_name}: Mismatched X and Y lengths.")
            else:
                print(f"[Warning] Skipping {row.attrib['name']} in {img_name}: Missing X or Y data.")

        if all_corners:
            print(f"[Info] Parsed {len(all_corners)} keypoints for {img_name}")
